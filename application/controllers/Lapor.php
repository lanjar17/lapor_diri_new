...
class Lapor extends CI_Controller {
	...
	public function view_super_admin_berkas($id_mahasiswa = NULL)
	{
	if ($this->session->userdata('logged_in') == true AND $this->session->userdata('id_user_level') == 3) {

		$data['berkas'] = $this->m_lapor->get_berkas($id_mahasiswa = $this->uri->segment(3))->result_array();
		$this->load->view('super_admin/detail_berkas', $data);
 	...
 	// public function view_super_admin_berkas($id_user) {
	// $data['berkas'] = $this->m_lapor->get_berkas($id_user);

	// // Memuat view dengan data berkas
	// $this->load->view('super_admin/detail_berkas', $data);
    // }
...
{
    // Memanggil model yang telah Anda buat
    $this->load->model('M_lapor'); // Ganti 'Nama_Model' dengan nama sebenarnya

    // Memanggil fungsi get_all_berkas() dari model
    $data['mahasiswa'] = $this->M_lapor->get_all_berkas();

    // Memuat view dan mengirim data ke view
    $this->load->view('Lapor/view_mahasiswa', $data); // Ganti 'nama_view' dengan nama view yang sesuai
}
	...
	public function edit_lapor_admin()
	{
		...
		$hasil = $this->m_lapor->update_lapor($nama, $id_mahasiswa);
