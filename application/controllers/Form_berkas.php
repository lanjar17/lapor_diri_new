...
class Form_Berkas extends CI_Controller
{
	...
	public function view_berkas()
	{
		if ($this->session->userdata('logged_in') == true and $this->session->userdata('id_user_level') == 1) {
			...
			$this->load->view('mahasiswa/form_pengajuan_berkas', $data);
		} else {
    ...
    if ($this->session->userdata('logged_in') == true and $this->session->userdata('id_user_level') == 1) {
        ...
        $id_berkas = md5($id_user);
        $id_status_mahasiswa = 1;

        $hasil = $this->m_lapor->insert_data_berkas(
            'mahasiswa-' . substr($id_berkas, 0, 5),
            $id_user,
            $dokumen_a1,
            $dokumen_pakta,
            $dokumen_biodata,
        ...
        redirect('Form_Berkas/view_berkas');
    } else {
        $this->session->set_flashdata('loggin_err', 'loggin_err');
        redirect('Login/index');
    }
